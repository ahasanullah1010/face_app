        ...
        <div class="col-md-8 mx-auto">
            ...
            <?php while ($post = $result->fetch_assoc()): ?>
                ...
                <div class="post-card">
                    ...
                    <div class="mt-3">
                        <form action="tmp_timeline.php" method="POST" class="d-inline">
                    ...
                    <div class="collapse comment-section mt-3" id="comments-<?= $post['post_id'] ?>">
                        ...
                        <!-- Add Comment Form -->
                        <form action="tmp_timeline.php" method="POST" class="mt-3">
...
// $query = "SELECT p.*, u.full_name, u.profile_picture FROM posts p JOIN users u ON p.user_id = u.user_id  ORDER BY p.created_at DESC";
                ...
                <?php while ($post = $result->fetch_assoc()): ?>
                    <div class="post-card">
                        <div class="d-flex align-items-center mb-3">
                            <img src="<?= $post['profile_picture'] ?: 'default_profile.jpg' ?>" alt="Profile Picture" class="rounded-circle" width="50" height="50">
                            <div class="ms-3">
                                <h5 class="mb-0"><?= $post['full_name'] ?></h5>
                                <small class="text-muted"><?= date("F j, Y, g:i a", strtotime($post['created_at'])) ?></small>
                            </div>
                        </div>
                        <p><?= $post['content'] ?></p>
